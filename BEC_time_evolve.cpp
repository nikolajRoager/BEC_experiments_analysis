...
int main(int argc, char** argv)
{
    ...
    std::cout<<"BEC time evolution"<<std::endl;
    ...
    for (unsigned int i = 0; i*2+1 < argc; ++i)
    {
        ...
        try
        {
            ...
            else if ( std::string(argv[i*2+1]).compare("mode")==0 )
            {
                if (std::string(argv[i*2+2]).compare("shake")==0)
                {
                    shake = true;
                    squeeze = false;
                }
                else if (std::string(argv[i*2+2]).compare("squeeze")==0)
                {
                    squeeze = true;
                    shake = false;
                }
                else if (std::string(argv[i*2+2]).compare("both")==0)
                {
                    squeeze = true;
                    shake = true;
    ...
    auto n_print = n_steps/64;
    auto time = makeTimeControl((n_steps), dt, 0.0);

    //Lets try out the make-control function, this way we can use multiple control parameters
    auto xc = 0.1*(shake? 1 : 0)*sin(time*omega_x);//offset
    auto sq = 1+0.2*(squeeze? 1 : 0)*sin(2*time*omega_x);//squeezing

    auto u = makeControl({xc,sq});
    ...
    [&x,amplitude,sigma2](RVec u)
    {
        return -amplitude*(exp(-2*pow(x-u.at(0),2)/(pow(u.at(1),2)*sigma2)));
    },u.getFront()
    );
    ...
    for (count_t R = 0; R < repeats; ++R)
        for (count_t N : Ns)
        {
            auto T0 = std::chrono::high_resolution_clock::now();
            tell_time(std::to_string(R+1)+"/"+std::to_string(repeats)+" N="+std::to_string(N));
            dc["N"].append(N);
            //Get wavefunctions

            auto H_dynamic = s.T()+V_dynamic+makeGpeTerm(g_per_N*N);

            tell_time("Setting up starting wavefunctions");
            tell_time("0/3");
            auto psi_init = makeWavefunction(H_dynamic(u.getFront())[0]);//Normalize is on by default
            tell_time("1/3");
            auto psi_1 = makeWavefunction(H_dynamic(u.getFront())[1]);
            tell_time("2/3");
            auto psi_2 = makeWavefunction(H_dynamic(u.getFront())[2]);
            tell_time("3/3 ... simulating from 0 ms to "+std::to_string(duration)+" ms");

            //I could get more states, but that would be stretching the GPE a little too far
            ...
            //set up the solver
            auto solver = makeFixedTimeStepper(H_dynamic,psi_init,dt);
            ...
            for(auto i=0; i < n_steps; i++)
            {
                ...
                dc["psi2_"+std::to_string(num)].append(solver.state().vec().absSquare());

                dc["Z0_"+std::to_string(num)].append(fidelity(psi_init,solver.state()));
                dc["Z1_"+std::to_string(num)].append(fidelity(psi_1,solver.state()));
                dc["Z2_"+std::to_string(num)].append(fidelity(psi_2,solver.state()));
                if (num==0 && R==0)
                    dc["t"].append(t);
